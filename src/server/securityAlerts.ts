...
const summaryStore = memoizeByKey<RepoSecuritySummary>(CACHE_TTL_MS, async (repo) => {
...
export function fetchRepoSecuritySummary(repo: string): Promise<RepoSecuritySummary> {
  return summaryStore.get(repo);
}
