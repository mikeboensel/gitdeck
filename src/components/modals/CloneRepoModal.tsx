...
import { ChevronIcon, FolderIcon } from "../common/Icons";
...
export function CloneRepoModal({ repo, onClose }: CloneRepoModalProps) {
  ...
  // Block dismissal (backdrop / Esc / close button) while a clone is in flight.
  const requestClose = () => {
    if (!cloning) onClose();
  };
    ...
    >
      <div className="clone-body">
        ...
        <ul className="clone-dirs">
          ...
          {!loading && !entries.length ? <li className="clone-empty">{t("clone.empty")}</li> : null}
        </ul>
