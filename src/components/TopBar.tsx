  ...
  return (
    <div className="topbar">
      <div className="topbar-orgs">
        {visibleOrgs.map((org) => (
          <span className="topbar-org" data-tip={org} key={org}>
            <img className="topbar-org-icon" src={`https://github.com/${org}.png?size=80`} alt={org} />
          </span>
        ))}
        {overflowOrgs > 0 ? (
      ...
      <div className="topbar-actions">
        ...
        <button className="btn" aria-label={t("common.refresh")} title={t("common.refresh")} disabled={loading} onClick={onRefresh}>
        ...
        <div className="profile-menu" ref={profileRef}>
          <button
            ...
            type="button"
            aria-label={authLogin ? `${t("common.signedIn")} ${authLogin}` : t("common.authenticated")}
            aria-haspopup="menu"
          ...
          >
            ...
            <svg className="profile-caret" width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9" /></svg>
          </button>
          {profileOpen ? (
            <div className="profile-popover" role="menu" aria-label={t("common.account")}>
                ...
                <div className="profile-accounts" role="group" aria-label={t("accounts.switch")}>
                  {accounts.map((account) => {
                    ...
                    return (
                      ...
                      >
                        <Avatar login={account.login ?? undefined} size={28} className="profile-account-avatar" />
                        ...
                        {isActive ? (
                          <svg className="profile-account-check" width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5" /></svg>
                        ) : !account.ephemeral ? (
                          <button
                            type="button"
                            ...
                            onClick={(event) => void handleRemoveAccount(event, account.id, labelText)}
                          >
                            ×
                          </button>
                        ) : null}
                  ...
                  <button
                    ...
                    role="menuitem"
                    onClick={() => { setProfileOpen(false); setAddAccountOpen(true); }}
                  >
                    <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
              ...
              >
                ...
                <svg className={`profile-prefs-caret ${prefsOpen ? "open" : ""}`} width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9" /></svg>
              </button>
              {prefsOpen ? (
              ...
              {canLogout ? (
                <button className="profile-menu-item profile-logout" type="button" role="menuitem" onClick={() => { setProfileOpen(false); onLogout(); }}>
