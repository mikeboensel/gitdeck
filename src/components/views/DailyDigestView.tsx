...
function formatDigestDate(
  ...
  if (period === "week") {
    ...
    return t("digest.weekOf", {
      from: start.toLocaleDateString(undefined, fmt),
      to: d.toLocaleDateString(undefined, fmt),
    });
  }
  ...
  return (
    <div className="digest-wrapper">
      ...
      {!digests.length ? (
        <div className="empty">
          <div className="big">
            {t("digest.empty", { period: t(PERIOD_LABEL_KEYS[period]).toLowerCase() })}
          </div>
            ...
            <article className="digest-card" key={digest.date}>
              <div className="digest-head">
                ...
                <div className="digest-badges">
                  <span>
                    ★ {digest.starsDelta >= 0 ? "+" : ""}
                    {formatNumber(digest.starsDelta)}
                  </span>
                  <span>
                    {t("digest.forksDelta", {
                      value: `${digest.forksDelta >= 0 ? "+" : ""}${formatNumber(digest.forksDelta)}`,
                    })}
                  </span>
                  <span>
                    {t("digest.issuesDelta", {
                      value: `${digest.issueDelta >= 0 ? "+" : ""}${formatNumber(digest.issueDelta)}`,
                    })}
                  </span>
                  <span>
                    {t("digest.securityAlerts", {
                      count: formatNumber(digest.securityAlertsCount),
                    })}
                  </span>
                  <span>
                    {t("digest.securityRepos", { count: formatNumber(digest.securityReposCount) })}
                  </span>
                  <button
                    type="button"
                  ...
                  >
                    {t("digest.copyMarkdown")}
                  </button>
                </div>
              </div>
              {digest.securityAlertsUnavailable ? (
                <div className="modal-info-banner">{t("digest.securityUnavailable")}</div>
              ) : null}
              {digest.ai ? (
              ...
              <div className="digest-pill-groups">
                <div className="digest-pill-group">
                  ...
                  {digest.executiveSummary.map((item) => (
              ...
              <div className="digest-repo-list">
                {digest.repos.map((repo) => (
                  <div className="digest-repo-row" key={`${digest.date}-${repo.repo}`}>
                    ...
                    <span>
                      {t("digest.forksDelta", {
                        value: `${repo.forksDelta >= 0 ? "+" : ""}${formatNumber(repo.forksDelta)}`,
                      })}
                    </span>
                    <em>
                      {t("digest.issuesDelta", {
                        value: `${repo.issueDelta >= 0 ? "+" : ""}${formatNumber(repo.issueDelta)}`,
                      })}
                    </em>
                    <em>
                      {t("digest.securityAlerts", {
                        count: formatNumber(repo.securityAlertsCount),
                      })}
                    </em>
                  </div>
                ))}
