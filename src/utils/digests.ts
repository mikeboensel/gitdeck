...
/** Format a delta with an explicit sign: 3 → "+3", -2 → "-2", 0 → "+0". */
...
function buildRepoDelta(current: DailyRepoRecord, previous?: DailyRepoRecord): DailyRepoDigest {
  ...
  const risks = [
    issueDelta > 0 ? `Open issues ${signed(issueDelta)}.` : "",
    staleIssueDelta > 0 ? `Stale issues ${signed(staleIssueDelta)}.` : "",
    current.securityAlertsCount > 0
      ? `Security alerts ${signed(current.securityAlertsCount)}.`
      : "",
  ].filter(Boolean);
  const momentum = [
    starsDelta > 0 ? `Stars ${signed(starsDelta)}.` : "",
    forksDelta > 0 ? `Forks ${signed(forksDelta)}.` : "",
  ].filter(Boolean);
  ...
  return {
    ...
    securityAlertsDelta,
    highlights: [
      `Stars ${signed(starsDelta)}, forks ${signed(forksDelta)}, open issues ${signed(issueDelta)}.`,
      current.securityAlertsCount > 0
        ? `Security alerts ${signed(current.securityAlertsCount)}.`
        : "",
    ],
    executiveSummary: [
      ...
      `Daily movement: stars ${signed(starsDelta)}, forks ${signed(forksDelta)}, issues ${signed(issueDelta)}${current.securityAlertsCount > 0 ? `, security alerts ${signed(current.securityAlertsCount)}` : ""}.`,
      risks[0] ||
        momentum[0] ||
        (current.securityAlertsCount > 0
...
/** Per-repo deltas for one record, sorted by largest absolute movement first. */
function buildSortedRepoDeltas(
  record: DailyDigestRecord,
  previous: DailyDigestRecord | undefined,
): DailyRepoDigest[] {
...
function buildDailyHighlights(
  record: DailyDigestRecord,
  previous: DailyDigestRecord | undefined,
  repoDeltas: DailyRepoDigest[],
): string[] {
  const starsDelta = record.totalStars - (previous?.totalStars || 0);
  const forksDelta = record.totalForks - (previous?.totalForks || 0);
  const issueDelta = record.issueCount - (previous?.issueCount || 0);
  ...
  return [
    ...
    record.securityAlertsCount > 0
      ? `Security alerts ${signed(record.securityAlertsCount)} across ${record.securityReposCount} repos.`
      : "",
    topIssueMover
      ? `${topIssueMover.repo} changed open issues by ${signed(topIssueMover.issueDelta)}.`
      : "",
    topStarMover ? `${topStarMover.repo} changed stars by ${signed(topStarMover.starsDelta)}.` : "",
    topStaleMover
      ? `${topStaleMover.repo} changed stale issues by ${signed(topStaleMover.staleIssueDelta)}.`
      : "",
    topSecurityMover
...
function buildDigestEntry(
  ...
  const repoDeltas = buildSortedRepoDeltas(record, previous);
  const issueDelta = record.issueCount - (previous?.issueCount || 0);
  const staleIssueDelta = record.staleIssueCount - (previous?.staleIssueCount || 0);
  const starsDelta = record.totalStars - (previous?.totalStars || 0);
  const forksDelta = record.totalForks - (previous?.totalForks || 0);

  const highlights = buildDailyHighlights(record, previous, repoDeltas);
  ...
  const risks = repoDeltas
    ...
    .map(
      (repo) =>
        `${repo.repo}: issues ${signed(repo.issueDelta)}, stale ${signed(repo.staleIssueDelta)}, security ${signed(repo.securityAlertsCount)}.`,
    );
  ...
  const momentum = repoDeltas
    ...
    .slice(0, 3)
    .map(
      (repo) => `${repo.repo}: stars ${signed(repo.starsDelta)}, forks ${signed(repo.forksDelta)}.`,
    );

  const executiveSummary = [
    `Tracked ${record.repoCount} repositories with ${record.issueCount} open issues in total.`,
    `Daily deltas: stars ${signed(starsDelta)}, forks ${signed(forksDelta)}, issues ${signed(issueDelta)}${record.securityAlertsCount > 0 ? `, security alerts ${signed(record.securityAlertsCount)}` : ""}.`,
    risks[0] ||
      momentum[0] ||
      (record.securityAlertsCount > 0
...
export function buildDailyDigestEntries(records: DailyDigestRecord[]): DailyDigestEntry[] {
  ...
  return sorted.map((record, index) => buildDigestEntry(record, sorted[index + 1]));
}
