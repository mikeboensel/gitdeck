...
export function buildRepoInsight(input: RepoInsightInput): RepoInsight {
  ...
  const daysSinceRelease = latestReleasePublishedAt
    ? Math.floor((now - new Date(latestReleasePublishedAt).getTime()) / DAY_MS)
    : null;
  ...
  if (viewsCount > 0 && totalDownloads > 0)
    healthScore += Math.min(6, Math.floor((totalDownloads / Math.max(viewsCount, 1)) * 10));
  ...
  if (securityAlertsCount > 0)
    alerts.push(`${securityAlertsCount} open security alerts need attention.`);
  if (staleIssueCount >= 3) alerts.push(`${staleIssueCount} stale issues need attention.`);
  if (daysSincePush > 45 && repoIssues.length > 0)
    alerts.push(`No push for ${daysSincePush} days while issues remain open.`);
  if (viewsCount > 250 && (daysSinceRelease === null || daysSinceRelease > 120))
    alerts.push("Traffic is active but the release cadence is cold.");
  if (totalDownloads > 100 && staleIssueCount >= 2)
    alerts.push("Downloads are healthy, but maintenance debt is building up.");

  if (viewsCount > 200 && totalDownloads < 50)
    opportunities.push("Traffic is landing, but downloads are not converting yet.");
  if ((starsDelta ?? 0) >= 5 || (forksDelta ?? 0) >= 3)
    opportunities.push("Momentum is growing; this repo could use a stronger release push.");
  if (releaseCount === 0 && viewsCount > 100)
    opportunities.push("This repo gets attention without any formal releases.");
  if (repoIssues.length === 0 && viewsCount > 0 && totalDownloads > 0)
    opportunities.push("Healthy adoption with low support load; a good candidate for promotion.");
  ...
  if (viewsCount > 0 && recentDownloads > 0)
    correlations.push("Traffic is converting into recent release downloads.");
  if (viewsCount > 0 && totalDownloads > viewsCount)
    correlations.push("Downloads exceed the last 14 days of views, suggesting long-tail adoption.");
